...
def generate_aspherical(R, k=0.0,
    ...
    if bound_type == 1:
        ...
        if full_width <= 0 or full_length <= 0:
            raise ValueError("全口径的 X/Y 尺寸必须为正数")
        xmin, xmax = -full_width / 2, full_width / 2
        ymin, ymax = -full_length / 2, full_length / 2
        in_local = lambda x, y: (rect_xmin - 1e-6 <= x <= rect_xmax + 1e-6 and
        ...
        xmin, xmax = -full_width / 2, full_width / 2
        ymin, ymax = -full_length / 2, full_length / 2
        in_local = lambda x, y: (x - circ_xc) ** 2 + (y - circ_yc) ** 2 <= circ_R ** 2 + 1e-6

    # Local coverage is a crop of this complete-aperture trajectory.  It must
    # not restart the raster/spiral phase at the local region's boundary.
    if traj_type == "G":
        p2d = generate_surface_raster(
            xmin, xmax, ymin, ymax, direction, step_len, line_spacing,
            lambda x, y: sag_and_gradient(x, y)[0], keep=in_local)
    else:
        R_max = 0.5 * np.hypot(full_width, full_length)
        raw = generate_spiral_2d(pitch, arc_step, R_max, 0.0, 0.0)
    ...
    result = []
    for x, y in p2d:
        Z, dZdX, dZdY = sag_and_gradient(x, y)
